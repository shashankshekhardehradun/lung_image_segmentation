...
if __name__ == '__main__':
    ...
    for xx, yy in test_gen.flow(X, y, batch_size=1):
        ...
        if (i<8):
            ...
            #cv2.imwrite('Processed%d.jpg'%(i),img)
            plt.imshow(img, cmap='gray')

            plt.subplot(8, 4, 4 * i+2)
            #plt.title('IoU = {:.4f}'.format(ious[i]))
            plt.axis('off') 
            #cv2.imwrite('mask%d.jpg'%(i), masked(img, gt, pr,1))
            ...
            #cv2.imwrite('diff%d.jpg'%(i),np.dstack((pr.astype(np.int8), gt.astype(np.int8), pr.astype(np.int8))))
        i += 1
        if i == n_test:
            break
